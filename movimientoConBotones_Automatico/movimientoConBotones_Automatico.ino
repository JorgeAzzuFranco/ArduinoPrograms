...
void mover(){
  //Front
  if((digitalRead(BF) == 1 and bf == 0) or direccion == 1){
    bf = 1;
    direccion = 1;
    posY--;
    posY = constrain(posY,0,vel);
  }
  ...
  //Back
  if((digitalRead(BB) == 1 and bb == 0) or direccion == 2){
    bb = 1;
    direccion = 2;
    posY++; 
    posY = constrain(posY,0,vel);
  ...
  //Left
  if((digitalRead(BL) == 1 and bl == 0) or direccion == 3){
    bl = 1;
    direccion = 3;
    posX--; 
    posX = constrain(posX,0,vel);
  }
  ...
  //Right
  if((digitalRead(BR) == 1 and br == 0) or direccion == 4){
    br = 1;
    direccion = 4;
    posX++; 
    posX = constrain(posX,0,vel);
  ...
  //Up
  if((digitalRead(BU) == 1 and bu == 0) or direccion == 5){
    bu = 1;
    direccion = 5;
    posZ++; 
    posZ = constrain(posZ,0,vel);
  }
  ...
  //Down
 if((digitalRead(BD) == 1 and bd == 0) or direccion == 6){
    bd = 1;
    direccion = 6;
    posZ--; 
    posZ = constrain(posZ,0,vel);
